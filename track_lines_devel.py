...
def line_supersample(df):
    ...
    return np.stack( (
            np.interp(xnew, x, lon), np.interp(xnew, x, lat),
            np.interp(xnew, x, ff), np.interp(xnew, x, pt),
        ), axis=-1)
...
def find_best_match(i0ref, line0, i1ref, line1):
    ...
    for ioff in range(-25,27):
        ...
        if i1 < 0 or i1 >=line1.shape[0]:
            continue
        while i0 > 0 and i1 > 0:
            dist = dynlib.utils.dist_sphere(line0[i0,0], line0[i0,1], line1[i1,0], line1[i1,1])
            i0 -= 1
            i1 -= 1
...
def track_lines(df0, df1, debug=False):
    ...
    dists = dynlib.utils.dist_sphere(
            lons0s[:,np.newaxis], lats0s[:,np.newaxis], 
            lons1[np.newaxis,:], lats1[np.newaxis,:]
    )
    ...
    for pidx0s, pidx1 in enumerate(match):
        ...
        line_match_[lidx1_] = (dists[pidx0s], pidx0+df0_firstidx, pidx1+df1_firstidx)
    ...
    if debug:
        ...
        for lidx0 in set(lidx0s):
            ...
            m.plot(line.longitude.to_numpy(), line.latitude.to_numpy(), 'k', linewidth=2, zorder=3, latlon=True)
        for lidx1 in set(lidx1):
            line = df1[df1.line_id == lidx1]
            m.plot(line.longitude.to_numpy(), line.latitude.to_numpy(), 'b', linewidth=2, zorder=3, latlon=True)
    ...
    for lidx0, matching_lines1 in line_match.items():
        ...
        for lidx1, (dist, pidx0, pidx1) in matching_lines1.items():
            ...
            if l0overlap.shape[0] == 0:
                l0overlap, l1overlap = find_best_match(i0ref, l0super, l1super.shape[0]-1-i1ref, l1super[::-1,:])
            
            if l0overlap.shape[0] > 0:
                N += l0overlap.shape[0]
                overlaps.append(
                    np.concatenate((l0overlap, l1overlap), axis=1)
                )

                matches.append((lidx0, lidx1))

                if debug:
                    m.plot(l0overlap[:,0], l0overlap[:,1], 'r', linewidth=4, zorder=2, latlon=True)
                    m.plot(l1overlap[:,0], l1overlap[:,1], 'r', linewidth=4, zorder=2, latlon=True)
...
periods = [(yr, mon) for yr in range(1979,2023) for mon in range(1,13)] # must be sequential for tracking across boundaries
periodstr = '1979-2022'
#periods = [(1979,1), ]
#periodstr = '197901'
...
if __name__ == '__main__':
    if debug_one is None:
        ...
        for yr, mon in periods:
            df = xr.open_dataset(f'{ipath}/ea.ans.{yr:04d}{mon:02d}.pv2000.jetaxis.nc').to_dataframe()
            add_length_col(df)
            ...
            if prev_ts is not None:
                ...
                if (dates[0] - prev_ts.date.iloc[0]) > pd.Timedelta(6, 'hours'):
                    print(f'Warning: NOT tracking lines between {prev_ts.date.iloc[0].strftime("%Y%m%d %H")} and '
                           '{dates[0].strftime("%Y%m%d %H")}, as more than 6 hours are between them.')
                else:
                    print(f'Tracking lines from from {prev_ts.date.iloc[0].strftime("%Y%m%d %H")}.')

                    matches_, overlaps_, N = track_lines(prev_ts, df[df.date == dates[0]], debug=debug_plot)
                    update_graph(jet_graph, prev_ts.date.iloc[0], dates[0], set(prev_ts.line_id), matches_)
                    dateso.extend([prev_ts.date.iloc[0],]*N)
            ...
            for date0, date1 in zip(dates[:-1], dates[1:]):
                ...
                matches_, overlaps_, N = track_lines(df[df.date == date0], df[df.date == date1], debug=debug_plot)
                update_graph(jet_graph, date0, date1, set(df[df.date == date0].line_id), matches_)
                dateso.extend([date0,]*N)
                overlaps.extend(overlaps_)
            ...
            dfo.to_xarray().to_netcdf(f'{ipath}/ea.ans.{yr:04d}{mon:02d}.pv2000.jetaxis_connections.nc')
        ...
        matches_, overlaps_, N = track_lines(df[df.date == date0], df[df.date == date1], debug=True)
